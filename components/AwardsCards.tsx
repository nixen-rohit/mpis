      ...
      {cards.map((card) => {
        ...
        return (
          <motion.div
            ...
            className={`group relative overflow-hidden rounded-4xl border ${card.border} bg-white/80 p-5 sm:p-8 shadow-sm backdrop-blur-xl transition-all duration-500 hover:shadow-2xl`}
          >
            {/* Hover Glow */}
            <motion.div
              initial={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="absolute -right-10 -top-10 h-32 w-32 sm:h-40 sm:w-40 rounded-full bg-white/40 blur-3xl"
            />
            ...
            <div className="relative z-10 flex flex-col gap-5 sm:gap-6 md:flex-row md:items-start">
              ...
              <div>
                <motion.span
                  ...
                  className="mb-3 inline-flex rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-[10px] sm:text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 backdrop-blur-sm"
                >
                  {card.subtitle}
                </motion.span>
