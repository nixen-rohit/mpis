...
const getAlign = (index: number, total: number): "left" | "right" | "center" => {
...
export function DesktopNav({ navItems, className }: DesktopNavProps) {
  return (
    <nav className={cn("items-center gap-0.5 flex-1 justify-center", className)}>
      {navItems.map((item, i) => (
        <div key={i} className="relative group">
          <Link
            href={item.href ?? "#"}
            target={item.redirect && !item.mega ? "_blank" : undefined}
            rel={item.redirect && !item.mega ? "noopener noreferrer" : undefined}
          ...
          >
            {item.label}
            {item.redirect && !item.mega && <RiExternalLinkLine className="text-[11px] text-gray-400 ml-0.5" />}
            {item.mega && <RiArrowDownSLine className="text-gray-400 text-[15px] group-hover:rotate-180 transition-transform duration-200" />}
          </Link>
          {item.mega && <MegaMenuPanel mega={item.mega} align={getAlign(i, navItems.length)} />}
        </div>
      ))}
    </nav>
  );
