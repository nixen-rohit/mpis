...
export type FAQData = Record<string, FAQItemProps[]>;
...
const FAQHeader = ({ title, subtitle }: FAQHeaderProps) => (
  <div className="relative z-10 flex flex-col items-center justify-center">
    <span className="mb-8 bg-linear-to-r from-primary to-primary/60 bg-clip-text font-medium text-transparent">
      {subtitle}
    </span>
        ...
        <AnimatePresence>
          {selected === key && (
            <motion.span
              ...
              className="absolute inset-0 z-0 bg-linear-to-r from-primary to-primary/80"
            />
          )}
        </AnimatePresence>
      </button>
...
const FAQList = ({ faqData, selected }: FAQListProps) => (
  <div className="mx-auto mt-12 max-w-3xl">
    <AnimatePresence mode="wait">
      {(Object.entries(faqData) as [string, FAQItemProps[]][]).map(([category, questions]) => {
        if (selected === category) {
          return (
            <motion.div
              ...
              className="space-y-4"
            >
              {questions.map((faq: FAQItemProps, index: number) => (
